...
All notable changes to the TrenTorch software package are documented here.
...
1.0 — TrenTorch is still pedagogical-first and APIs may change between
...
### Added
- **Licensing clarity.** TrenTorch is now distributed under MIT (replaces
...
- `trentorch/__init__.py` reads its version from `pyproject.toml` at import
  time, so `import trentorch; trentorch.__version__` reflects the new
