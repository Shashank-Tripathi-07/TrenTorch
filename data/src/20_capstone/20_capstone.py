...
def test_unit_submission_with_optimization():
    ...
    submission = generate_submission(
        baseline_report, optimized_report, student_name="Test Student", techniques_applied=techniques
    )
...
def test_unit_improvements_calculation():
    ...
    assert abs(improvements["compression_ratio"] - 2.0) < 0.01, (
        f"Expected compression 2.0, got {improvements['compression_ratio']}"
    )

    # Accuracy delta = 0.75 - 0.80 = -0.05
    assert abs(improvements["accuracy_delta"] - (-0.05)) < 0.001, (
        f"Expected accuracy delta -0.05, got {improvements['accuracy_delta']}"
    )
...
def test_unit_json_serialization():
    ...
    assert loaded_json["baseline"]["model_name"] == submission["baseline"]["model_name"], (
    ...
    assert baseline_metrics["parameter_count"] == original_metrics["parameter_count"], (
