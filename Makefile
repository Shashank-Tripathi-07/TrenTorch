...
preflight:
	$(PYTHON) -m platforms.cli.main dev preflight

# Quick preflight (faster)
preflight-quick:
	$(PYTHON) -m platforms.cli.main dev preflight --quick
...
release:
	$(PYTHON) -m platforms.cli.main dev preflight --release
...
# Lint code
lint:
	$(PYTHON) -m py_compile platforms/cli/main.py
...
ci-smoke:
	$(PYTHON) -m platforms.cli.main dev preflight --quick --ci

# CI standard test (for PRs)
ci-standard:
	$(PYTHON) -m platforms.cli.main dev preflight --ci
	$(PYTHON) -m pytest tests/e2e/ -k quick --tb=short -q

# CI full test (for releases)
ci-full:
	$(PYTHON) -m platforms.cli.main dev preflight --full --ci
	$(PYTHON) -m pytest tests/ -v --ignore=tests/milestones --tb=short

# CI release validation (comprehensive)
ci-release:
	$(PYTHON) -m platforms.cli.main dev preflight --release --ci

# CI JSON output (for automation/parsing)
ci-json:
	$(PYTHON) -m platforms.cli.main dev preflight --json
