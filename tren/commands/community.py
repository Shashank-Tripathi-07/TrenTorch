...
class CommunityCommand(BaseCommand):
    ...
    def _show_status(self) -> int:
        ...
        if is_logged_in:
            ...
            self.console.print(Panel(
                ...
                "  [bold green]tren community login[/bold green]",
    ...
    def _sync(self) -> int:
        ...
        'tren community sync' to push their current progress.json at any time.
        """
        if not auth.is_logged_in():
            self.console.print("[yellow]You are not logged in.[/yellow] Run [bold green]tren community login[/bold green] first, then sync.")
            return 1
