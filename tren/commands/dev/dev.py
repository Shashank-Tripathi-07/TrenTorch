    ...
    def run(self, args: Namespace) -> int:
        ...
        if not hasattr(args, 'dev_command') or not args.dev_command:
            console.print(Panel(
                ...
                "  [dim]tren dev build html[/dim]         Build HTML site\n"
                "  [dim]tren dev build serve[/dim]        Build and serve locally\n"
                "  [dim]tren dev build pdf[/dim]          Build PDF course guide\n"
                "  [dim]tren dev build paper[/dim]        Build research paper\n\n"
                "[bold cyan]Clean:[/bold cyan]\n"
                "  [dim]tren dev clean[/dim]              Clean all generated files\n"
                "  [dim]tren dev clean site[/dim]         Clean site build artifacts\n\n"
