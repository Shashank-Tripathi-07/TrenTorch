...
class NBGraderCommand(BaseCommand):
    ...
    def run(self, args: Namespace) -> int:
        ...
        if not getattr(args, "nbgrader_command", None):
            console.print(Panel(
                ...
                "[dim]Examples:[/dim]\n"
                "[dim]  tren nbgrader init[/dim]\n"
                "[dim]  tren nbgrader generate 01[/dim]\n"
                "[dim]  tren nbgrader generate --all[/dim]\n"
                "[dim]  tren nbgrader generate --range 01-04[/dim]\n"
                "[dim]  tren nbgrader release 01_tensor[/dim]\n"
                "[dim]  tren nbgrader autograde 01_tensor --student student_id[/dim]",
    ...
    def _ensure_module_notebook(self, module_name: str) -> Optional[Path]:
        ...
        `tren module start`/`resume` open for students to edit. This method
