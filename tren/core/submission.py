...
class SubmissionHandler:
    ...
    def sync_progress(self, total_modules: int = 20, is_retry: bool = False) -> SyncResult:
        ...
        if not token:
            self.console.print("❌ [bold red]You are not logged in.[/bold red] Please run 'tren community login' first.")
        ...
        try:
            ...
            if e.code == 401 and not is_retry:
                ...
                if new_token:
                    ...
                    self.console.print("   Run 'tren community login --force' to refresh.")
    ...
    def _interpret_2xx(self, synced: Optional[int], sent: int) -> SyncResult:
        ...
        self.console.print("   [dim]Your dashboard may still show old progress. Re-run [bold]tren community sync[/bold];[/dim]")
...
def auto_sync_after_completion(config: CLIConfig, console: Console, *,
    ...
    if runtime.is_interactive():
        if not Confirm.ask(f"[bold yellow]{prompt}[/bold yellow]", default=True):
            console.print("[dim]Skipped. Run 'tren community sync' anytime to upload your progress.[/dim]")
            return None
    else:
