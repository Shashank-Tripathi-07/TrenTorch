...
class TestSigmoidActivation:
    ...
    def test_sigmoid_forward(self):
        ...
        STUDENT LEARNING: sigma(0) = 0.5 is a key property.
    ...
    def test_sigmoid_symmetry(self):
        """
        WHAT: Verify sigma(-x) = 1 - sigma(x) (point symmetry around 0.5).
    ...
    def test_sigmoid_derivative_property(self):
        """
        WHAT: Verify sigma'(x) = sigma(x) * (1 - sigma(x)).

        WHY: This elegant derivative formula makes backprop efficient.
        No need to store x - just use the output.

        STUDENT LEARNING: Maximum derivative is at x=0 where sigma'(0) = 0.25.
...
class TestTanhActivation:
    ...
    def test_tanh_forward(self):
        ...
        STUDENT LEARNING: tanh(0) = 0 (unlike sigmoid where sigma(0) = 0.5).
...
class TestSoftmaxActivation:
    ...
    CONCEPT: softmax(x_i) = e^(x_i) / sum(e^(x_j))
    ...
    def test_softmax_properties(self):
        ...
        cancels out in the ratio e^(x+c) / sum(e^(x+c)).
        """
        softmax = Softmax()
