...
def parse_requirements_file(filepath: Path) -> list[tuple[str, str | None, str | None]]:
    """
...
def check_version_compatibility(installed_version: str, version_spec: str | None) -> bool:
    """
...
class TestRequiredPackages:
    ...
    def test_package_installed(self, package_name):
        ...
        try:
            ...
            if version_spec and version != "unknown":
                ...
                assert is_compatible, f"{package_name} version {version} does not match {version_spec}"
    ...
    def test_package_functionality(self, package_name):
        ...
        if not success:
            pytest.fail(f"❌ {package_name} functionality test failed: {message}")
        ...
        for req_file in REQUIREMENTS_FILES:
            ...
            for pkg_name, version_spec, original_line in packages:
                ...
                package_specs[pkg_name].append(
                    {"file": req_file, "spec": version_spec, "line": original_line}
                )
        ...
        for pkg_name, specs in package_specs.items():
            if len(specs) > 1:
                ...
                if len(unique_specs) > 1:
                    conflicts.append({"package": pkg_name, "specs": specs})
