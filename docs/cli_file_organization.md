...
| `system/update.py` | 438 | `tren system update`. Checks GitHub for a newer tag, updates in place, preserves `data/modules/`, `trentorch/core/`, `user_data/`, `.venv/`. **Known issue**: still points at the upstream repo's tags, not this fork's. |
