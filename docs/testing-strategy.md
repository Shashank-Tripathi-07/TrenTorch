...
As of this writing: 20 curriculum modules, ~53K lines of curriculum source, ~13K lines of CLI source, 90 test files, roughly 1,080 test functions, and a baseline of 858 passing / 41 known-failing / 33 skipped on a clean run (the 41 are pre-existing, tracked failures, not something a green CI run is expected to eliminate on its own).
