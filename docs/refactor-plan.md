...
|---|---|---|
| `data/modules/01_tensor` .. `20_capstone` | The learner-facing curriculum: stub-only notebooks converted from `src/`, instructional cells plus a `NotImplementedError` stub for each exercise (no solution cell), plus `test_unit_*` / `test_module` assertions. | Generated *from* `src/` by `tren module start`. A student's own filled-in code, not any reference implementation, is what `tren module complete` tests and exports. |
| `data/solutions/01_tensor` .. `20_capstone` | Reference implementations, one per module, fully solved. | Also generated *from* `src/`. Gitignored, maintainer/CI-only: never surfaced by a command a student runs. Used to rebuild the ground-truth `trentorch/` package (`tren dev export`) and to verify curriculum integrity (`tren dev test --inline`), independent of any student's actual progress. |
| `src/01_tensor` .. `20_capstone` | The developer-authored source of truth for each module, plain Python with cell markers, each solvable exercise as a stub-cell/solution-cell pair. | This is the actual thing a contributor edits. `data/modules/`, `data/solutions/`, and `trentorch/` are all downstream of this. |
...
4. **Give Process explicit input/output declarations.** Even a lightweight comment convention at the top of each `tren/commands/*.py` command ("reads: src/, .tren/progress.json; writes: trentorch/, data/modules/") would have caught the Stage 7 dependency-width bug before it shipped, not after a profiling session found it.
