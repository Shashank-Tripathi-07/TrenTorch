...
| nbgrader | A Jupyter-based assignment and grading tool. | Provides the actual solution-stripping and grading engine. `tren nbgrader` is a thin, TrenTorch-specific convenience layer around nbgrader's own workflow (generate, release, collect, autograde, feedback). |
...
*The technologies below are the original TrenTorch project's own hosted infrastructure. The client-side code for them still exists in this fork's `tren/` and `quarto/community/`, but the backend endpoints it talks to belong to the upstream project, not TrenTorch, so this layer isn't functional here as-is.*
...
| A Netlify-hosted web backend | The TrenTorch website's own API, at `trentorch.netlify.app`. | Handles the browser-based login flow (`tren login` / `tren community login`) that authenticates a student before syncing progress or appearing on the community dashboard. |
| Vanilla HTML, CSS, and JavaScript | Plain web technologies, no framework. | The "TrenTorch Community" dashboard (`quarto/community/`), including an interactive 3D "globe" progress visualization, is hand-built HTML/JS served alongside the Quarto site rather than generated by it. |
...
Students can optionally create an account and opt into a social layer: `tren community login` opens a browser-based login flow against the TrenTorch website's backend, which redirects back to a short-lived local HTTP server with an access token. Once logged in, completing a module or milestone can automatically upload progress (via a Supabase Edge Function) to power a personal profile and an interactive "TrenTorch Globe" dashboard, hand-built as static HTML and JavaScript under `quarto/community/` and served alongside the Quarto docs site rather than generated by it. **The website and backend this talks to are the upstream project's own** (`trentorch.netlify.app` and its Supabase project); this fork inherited the client-side code but not the backend, so the feature is present but non-functional here. This entire layer is opt-in either way; nothing about completing modules or milestones requires an account, and progress always exists locally first (`.tren/progress.json`, `.tren/milestones.json`) regardless of whether it's ever synced.
...
- **`trentorch/scripts/build-docs.sh` is a stale script** referencing a defunct Jupyter Book documentation pipeline that predates the current Quarto site. It is not invoked by any current CI workflow.
...
- **`quarto/tren/troubleshooting.qmd` uses a package path that doesn't exist.** Several of its examples (`from trentorch.nn import Linear`, `ls trentorch/nn/layers.py`) reference a `trentorch.nn` submodule; no `nn` directory exists anywhere under `trentorch/trentorch/`. The real path for that same class is `trentorch.core.layers`, matching every other document and the actual `MODULE_EXPORT_CHECKS` mapping in `tren/commands/milestone.py`.
