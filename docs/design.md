...
- **`platforms/dev_tools/scripts/build-docs.sh` is a stale script** referencing a defunct Jupyter Book documentation pipeline (`docs/_build`, `build-book.sh`) that predates the (now also removed) Quarto site. It is not invoked by any current CI workflow.
