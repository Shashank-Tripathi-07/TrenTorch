...
- A single CLI, `tren`, that is the primary interface for the whole course: starting and testing modules, running milestones, benchmarking a finished framework, and (optionally) syncing progress to a community dashboard.
...
- Free and open source (MIT license for the `tinytorch`/`tren` packages), deployable and forkable by anyone.
...
| nbgrader | A Jupyter-based assignment and grading tool. | Provides the actual solution-stripping and grading engine. `tren nbgrader` is a thin, TinyTorch-specific convenience layer around nbgrader's own workflow (generate, release, collect, autograde, feedback). |
...
*The technologies below are the original TinyTorch project's own hosted infrastructure. The client-side code for them still exists in this fork's `tren/` and `quarto/community/`, but the backend endpoints it talks to belong to the upstream project, not TrenTorch, so this layer isn't functional here as-is.*

| Technology | What it is | How TinyTorch uses it |
|---|---|---|
| Supabase Edge Functions | Serverless functions backing a Supabase (Postgres-based) backend. | Receives progress and milestone data uploaded by `tren community sync` and the auto-sync flow after module completion. |
| A Netlify-hosted web backend | The TinyTorch website's own API, at `tinytorch.netlify.app`. | Handles the browser-based login flow (`tren login` / `tren community login`) that authenticates a student before syncing progress or appearing on the community dashboard. |
...
| Technology | What it is | How TinyTorch uses it |
|---|---|---|
| VS Code Extension API (TypeScript) | The API used to build Visual Studio Code extensions. | Powers "TinyTorch Workbench" (`vscode-ext/`), a sidebar extension that wraps the `tren` module, test, and build workflow in a graphical tree view, for students and contributors who prefer working inside VS Code. |
...
`tren` (entry point `tren.main:main`, declared in `pyproject.toml`) is a hand-built `argparse` application, not built on a third-party CLI framework. `TinyTorchCLI` (`tren/main.py`) keeps a single dictionary mapping top-level command names to command classes, and each top-level command is itself a command group that registers its own nested subcommands. Every command inherits from an abstract `BaseCommand` that provides consistent error handling and console access.

Top-level command groups:

| Command | What it's for |
|---|---|
| `tren setup` | First-time environment setup: creates a virtual environment, installs the fixed toolchain (NumPy, Jupyter, Jupytext, nbdev, Rich, PyYAML, psutil), registers a `tinytorch` Jupyter kernel, and prompts to create a local profile and (optionally) join the community. |
...
| `tren dev` | Developer and CI tooling, gated behind the `dev` group: `test` (the unified pytest runner used by CI), `preflight` (pre-release verification), `export` (rebuild the whole curriculum from `src/`), `build` (build the site, PDF guide, or paper via `make`), `clean`. |
...
| `tren nbgrader` | The instructor grading workflow: `init`, `generate`, `release`, `collect`, `autograde`, `feedback`, `status`, `analytics`, `report`. A convenience layer that shells out to the real `nbgrader` CLI for most of the heavy lifting. |
...
### Community dashboard and progress sync (upstream-only)

Students can optionally create an account and opt into a social layer: `tren community login` opens a browser-based login flow against the TinyTorch website's backend, which redirects back to a short-lived local HTTP server with an access token. Once logged in, completing a module or milestone can automatically upload progress (via a Supabase Edge Function) to power a personal profile and an interactive "TinyTorch Globe" dashboard, hand-built as static HTML and JavaScript under `quarto/community/` and served alongside the Quarto docs site rather than generated by it. **The website and backend this talks to are the upstream project's own** (`tinytorch.netlify.app` and its Supabase project); this fork inherited the client-side code but not the backend, so the feature is present but non-functional here. This entire layer is opt-in either way; nothing about completing modules or milestones requires an account, and progress always exists locally first (`.tren/progress.json`, `.tren/milestones.json`) regardless of whether it's ever synced.
...
A root `conftest.py` runs a pre-flight check before any test executes: it verifies the `tinytorch` package was actually exported and importable, and fails fast with an instruction to run `tren dev export --all` if not, rather than letting every downstream test fail with a confusing import error.
...
- **`tren olympics` is a placeholder with no real functionality**, as described above, distinct from the working `tinytorch.olympics` capstone module.
...
- **`quarto/tren/troubleshooting.qmd` uses a package path that doesn't exist.** Several of its examples (`from tinytorch.nn import Linear`, `ls tinytorch/nn/layers.py`) reference a `tinytorch.nn` submodule; no `nn` directory exists anywhere under `tinytorch/tinytorch/`. The real path for that same class is `tinytorch.core.layers`, matching every other document and the actual `MODULE_EXPORT_CHECKS` mapping in `tren/commands/milestone.py`.
- **`quarto/tren/overview.qmd`'s "Complete Command Reference" is missing two entire top-level command groups and one real subcommand.** `tren package` (`reset`, `nbdev`) and `tren olympics` don't appear anywhere in the table, despite both being real, registered commands (see the command table earlier in this document). The table's "Module Commands" section also omits `tren module path`, a real subcommand (`path_parser` in `workflow.py`) used by editor/IDE integrations to resolve a module's file path.
