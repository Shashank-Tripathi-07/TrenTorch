...
- **`INSTRUCTOR.md` documents two `tren module status` flags that don't exist.** Its "Tracking Progress" section shows `tren module status --student student_id` and `tren module status --export class_progress.csv` as working commands for checking individual or class-wide progress. The real `status` subparser (`tren/platforms/processes/module_workflow/workflow.py`) takes no arguments at all beyond the bare command; both examples would fail with an argparse "unrecognized arguments" error if an instructor actually typed them. There is no per-student or CSV-export progress view anywhere in the codebase.
